...
def _gemini_request(contents: list, include_tools: bool = True) -> dict:
    ...
    url = (
        ...
        f":generateContent?key={LLM_API_KEY}"
    )
    payload = {
...
def _execute_tool(
    ...
    try:
        if name == "instant_recommend":
            ...
            return instant_recommend(merchant, float(amount), user_card_ids, category=category)

        elif name == "plan_trip":
            destination = args.get("destination", "")
            total_budget = args.get("total_budget", 0)
            breakdown = args.get("breakdown")
            return plan_trip(destination, float(total_budget), breakdown, user_card_ids)
...
def _trim_tool_result(result: dict) -> dict:
    """精簡 tool result 減少 token。只保留關鍵欄位。"""
